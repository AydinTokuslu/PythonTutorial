    ...
    while recete_k:
        ...
        if recete_no in recete_list:
            ...
            if sec == "1":
                ...
                ilac_ödeme += ilac_fiyatlari[recete_list[recete_no]]
                print(f"Yapacağınız toplam ödeme {ilac_ödeme} tldir.")
                sec = input("Ödeme yapmak için 1'i, başka bir ilaç almak için 2'yi tuşlayınız : ")
                if sec == "1":
                    odeme(ilac_ödeme)
                elif sec == "2":
                    recete_kontrol(ilac_ödeme)
                else:
...
def odeme(ilac_ödeme):
    print(f"Ödemeniz gereken toplam tutar : {ilac_ödeme}")
